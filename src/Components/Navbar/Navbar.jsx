...
const NavLinks = ({ vertical = false, onClick = () => {} }) => {
  const layout = vertical ? "flex-col items-start gap-6 px-6 py-4" : "flex-row gap-6";
  const textColor = vertical ? "text-black" : "text-white";

  return (
    <div className={`flex ${layout} ${textColor} font-medium text-lg`}>
      <NavLink to="/home" onClick={onClick}>Home</NavLink>
      <NavLink to="/about" onClick={onClick}>About</NavLink>
      <NavLink to="/portal" onClick={onClick}>Portal</NavLink>
...
<nav className=" top-0 z-50 w-full bg-[#800020] px-8 py-4 flex items-center justify-between">
<div className="flex items-center gap-2">
  <Logo />
  <div className="text-[#F7B0D2] leading-tight">
